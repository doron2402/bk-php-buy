...
********************************************************************************  */
	...
	$id = $HTTP_GET_VARS['id'];
	opensql();
		$result = mysql_query("SELECT * FROM `Coupons_Texts` WHERE `text_id`='".$id."' LIMIT 1;");
	...
	if ($_POST['edit'] == "1")
	{
		...
		mysql_query("UPDATE `Coupons_Texts` SET 
			...
			WHERE `Coupons_Texts`.`text_id` =".$id.";");
				
				mysql_query("OPTIMIZE TABLE `Coupons_Texts`");
					mysql_query("REPAIR TABLE `Coupons_Texts`");
						mysql_query("ANALYZE TABLE `Coupons_Texts`");
