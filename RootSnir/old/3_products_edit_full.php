...
*********************************************************************************  */
	...
	opensql_redpoint();
		...
		$result = mysql_query("SELECT * FROM `products` WHERE `entity_id`='".$id."'");
			...
			$sku				 = mysql_result($result,0,"sku");
			$name				 = mysql_result($result,0,"name");

			$thumbnail			 = mysql_result($result,0,"thumbnail");
			$big_image			 = mysql_result($result,0,"small_image");

			$category_ids		 = mysql_result($result,0,"category_ids");
			$category_ids		 = substr($category_ids, 1);
			$category_exp = explode(",", $category_ids);
	...
	if ($_POST['update'] == 1)
	{
			...
			opensql_redpoint();
				...
				`taktzir` = '".$taktzir."', 
				`params` = '".$param."'
				WHERE `products`.`entity_id` =".$id." LIMIT 1 ;");
			...
			add_to_logs("עדכון מוצר - ".$id."");

			echo '<h1>מוצר #'.$id.' עודכן בהצלחה!<br><br>
...
?>
	...
	function checkform ( form )
	{
		...
		if (form.name.value == '') { alert( "name" ); form.name.focus(); return false ; }
	...
	<center>
	 <h1>שינוי פרטים למוצר #<? echo $id; ?></h1>
	...
	<!-- ################### item infotmation ################### -->
	 <table cellpadding="2" cellspacing="2" width="600">
	  <tr>
	   ...
	   <table>
	    <tr><td>מק"ט</td><td><input type="text" name="sku" style="width:200px;" value='<? echo $sku; ?>'></td></tr>
	    <tr><td>שם המוצר</td><td><input type="text" name="name" style="width:200px;" value='<? echo $name; ?>'></td></tr>
	   ...
	   <td valign="top" align="left" width="300">
	    <iframe src="http://www.redpoint.co.il/upload/?pass=123&act=edit&thumb=<? echo $thumbnail; ?>&big_pic=<? echo $big_image; ?>" border="0" frameborder="0" width="300" height="220"></iframe>
	    <!--<img src="http://www.redpoint.co.il/thumbs/<? echo $thumbnail; ?>"><br>
		<div style="text-align:right;">החלף תמונה:</div>
	...
	<!-- ################### Full text ################### --> 
	 <table cellpadding="2" cellspacing="2" width="600">
	  <tr><td>סקירה מהירה</td><td><textarea name="short_description" id="text1" style="width:300px;"><? echo $short_description; ?></textarea></td></tr>
	  <tr><td>פירוט מלא</td><td><textarea name="taktzir" id="text2" style="width:300px;"><? echo $taktzir; ?></textarea></td></tr>
	</table>
	...
	<? $p_exp = explode("<RtB>", $params); ?>
	 <table cellpadding="2" cellspacing="2" width="600">
	  ...
	  <div style="text-align:left; width:600px;"><input type="submit" value="עדכן!" style="padding:5px;"></div>
